...
END $$;
 ...
 create table payouts (id bigint not null, amount real check (amount >= 0) not null, batch_id varchar(255) not null, created_date timestamp(6) not null, description varchar(255), last_modified timestamp(6) not null, payout_fee float(53) not null, payout_status varchar(255) not null, account_id bigint, transaction_id bigint, primary key (id));
 ...
 create table product_details (product_id bigint not null, version varchar(30) not null, status varchar(255), cover_image varchar(255), upload_date timestamp(6) not null, description varchar(100), detail_description varchar(1000), flagged boolean not null, instruction varchar(255), last_update timestamp(6) not null, name varchar(30), price float(53) not null, category_id integer, license_id integer, primary key (product_id, version));
 create table product_details_tag (product_id bigint not null, version varchar(30) not null, tag_id integer not null);
 ...
 create table transactions (id bigint not null, amount real check (amount >= 0) not null, created_date timestamp(6) not null, description varchar(255), last_modified timestamp(6) not null, paypal_id varchar(255) not null, status varchar(255) not null, cart_id bigint not null, transaction_fee_id integer not null, primary key (id));
 ...
 alter table if exists product_details add constraint FKs1c6jm7fowox2m4fx1hejc2t8 foreign key (license_id) references licenses;
