...
END $$;
 ...
 create table products (id bigint not null, active_version varchar(30) not null, draft boolean not null, enabled boolean, seller_id bigint not null,created_date timestamp(6), last_modified timestamp(6), primary key (id));
 ...
 create table contacts (id bigint not null,firstname varchar(255),lastname varchar(255), created_date timestamp(6) not null,phone varchar(255), message varchar(255) not null, account_id bigint not null, primary key (id));
 ...
 alter table if exists contacts add constraint FKluuimmb5jiriqdocjau58ju9l foreign key (account_id) references accounts;