...
export class ProfileComponent implements OnInit {
	...
	getUserInfo() {
		this.userService.getCurrentUserInfo().subscribe(
			data => {
				this.user = data;
				if (this.user != null && this.user.avatar != null && this.user.id != -1) {
					this.ProfileImage.setAttribute('src', 'http://localhost:9000/public/serveMedia/image?source=' + this.user.avatar.replace(/\\/g, '/'));
				} else {
